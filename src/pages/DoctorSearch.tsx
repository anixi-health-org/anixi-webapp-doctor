...
export const DoctorSearch: React.FC = () => {
  ...
  useEffect(() => {
    const fetchDoctors = async () => {
      try {
        console.log('[DoctorSearch] 📋 Fetching all doctors...');
        setLoading(true);
        setError(null);
        ...
        console.log(`[DoctorSearch] ✅ Found ${snapshot.size} doctors`);
        ...
        console.error('[DoctorSearch] ❌ Error:', errorMsg);
        setError(errorMsg);
      } finally {
        setLoading(false);
      }
  ...
  const handleSendRequest = async (doctorId: string) => {
    ...
    try {
      ...
      console.log(`[DoctorSearch] 📤 Sending request to doctor ${doctorId}...`);
