...
export const DailyAdherenceView: React.FC<DailyAdherenceViewProps> = ({
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'taken':
        return 'bg-anixi-card text-green-800 border-green-300';
      case 'missed':
        return 'bg-anixi-card text-red-800 border-red-300';
      case 'pending':
        return 'bg-anixi-card text-yellow-800 border-yellow-300';
      default:
        return 'bg-anixi-card text-gray-800 border-gray-300';
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between p-4 bg-anixi-card rounded-lg border border-blue-200">
        <button
      ...
      {dailyData?.moodEntries && dailyData.moodEntries.length > 0 && (
        <div className="p-4 bg-anixi-card border-2 border-purple-200 rounded-lg">
        ...
        {timeSlots.map((timeSlot) => (
          <div key={timeSlot} className="p-4 bg-anixi-card border-2 border-blue-200 rounded-lg">
      ...
      {dailyData?.vitals && (
        <div className="p-4 bg-anixi-card border-2 border-red-200 rounded-lg">
          <h3 className="font-semibold text-gray-900 mb-3">🏥 Vitals</h3>
          <div className="grid grid-cols-2 gap-3">
            {dailyData.vitals.heartRate && (
              <div className="p-3 bg-anixi-card rounded border border-red-100">
